...
class DccpTrainer(ABC):
    ...
    def __init__(
        ...
        - param `stopping_methods`:
          ...
          should stop.
          In this case, training ends by rolling back to the epoch with the best
          validation cost.
        - param `solver`:
        ...
        - param `verbose`:
          ...
          0: no logging, 1: basic logging / timing, 2: cvxpy.
          solve() set to verbose mode.
        - param `random_state`:
    ...
    def at_training_start(self, data_dim: int) -> None:
        ...
        For example, create and persist a list of additional variables to optimize.
        """

    def after_epoch(self) -> None:
        """
        Optional additional actions to take after each epoch, that is after cvxpy
        solve() and during wrapping code.
        """

    def at_training_end(self) -> None:
        """
        Optional actions to take when the training is over.
        Ran as the very last function call in train, meaning after `rollback_to_best` is
        called.
        """
    ...
    def save_best(self) -> None:
        ...
        Save the data generated by after_epoch somewhere accessible at the end of the
        training process.

        Used in the training loop to roll back to the best state of the model at the end
        of the loop.
        """
    ...
    def rollback_to_best(self) -> None:
        ...
        Restore the state saved in save_best, after the training process is done, to go
        back to the best performing model.
        """
    ...
    def get_problem_dccp(
        ...
        - param `X`:
          The data points.
        - param `y`:
          The data points classes.
        - param `cost_weights`:
          The wdccp weights, all 1 if not using wdccp, for use in computing the
          objective.
          This is an array with one element corresponding to an element in X.

        return: a tuple containing a cvxpy Objective and a list of Constraints.
        """
    ...
    def get_cost(self, X: np.ndarray, y: np.ndarray) -> float:
        """
        Compute the cost for a set of sample points in the same way as the cost defined
        in get_problem, used in train/validation training.

        If applicable, should use the weights computed in this epoch's `after_epoch`
        call, as it will have been made before this method's call.
        """
    ...
    def train_linearized(
        ...
        """
        Helper function for train, uses a loop to solve multiple approximated steps,
        updating linearization accuracy over time.
        """
    ...
    def train_dccp(
        ...
        """
        Helper function for train, uses the dccp library to solve the problem in one
        step.
        """
    ...
    def train(self, X: np.ndarray, y: np.ndarray) -> None:
        """
        Train using DCCP.
        This means this function can only solve problems where the cost function can be
        separable in the given convex and concave parts.
        ...
        # This must be fixed in derived classes as this behavior may not always
        # be problematic.
